...
'''
    Match topic using wildcard and return matched phrases.
        
    Here are examples:
        _match_topic('test/foo/bar', 'test/foo/bar') # []
        _match_topic('test/foo/bar', 'test/+/bar') # ['foo']
        _match_topic('test/foo/bar', 'test/#') # ['foo/bar']
        _match_topic('test/foo/bar/baz', 'test/+/#') # ['foo', 'bar/baz']
        _match_topic('test/foo/bar/baz', 'test/+/+/baz') # ['foo', 'bar']

        _match_topic('test', 'test/#') # []
        _match_topic('test/', 'test/#') # ['']

        _match_topic('test/foo/bar', 'test/+') # None
        _match_topic('test/foo/bar', 'test/nope/bar') # None
'''
def match_mqtt_topic(topic, wildcard):
    if topic == wildcard:
        return []
    elif wildcard == '#':
        return [topic]
    ...
    for i in range(len(w)):
        if w[i] == '+':
            ...
            res.append('/'.join(str(e) for e in t[i:]))
            return res
        elif i==len(t) or w[i] != t[i]:
            return None
...
config = {
    ...
    "queue_len": 1,
    "gateway" : False,
    "topics": [] # in form of [("topic1", cb1), ("topic1", cb2)]
}
...
class MQTT_base:
    ...
    def __init__(self, config):
        ...
        # Topic subscriptions
        self._topic_prefix = ''
        if self.server.lower() == 'mqtt.ohstem.vn' or self.server.lower() == 'io.adafruit.com':
            self._topic_prefix = '{:s}/feeds/'.format(self._user)

        self._topics = []

        for t, cb in config["topics"]:
            self._topics.append((self._topic_prefix + t, cb))
...
class MQTTClient(MQTT_base):
    ...
    async def wifi_connect(self, quick=False):
        ...
        if ESP8266:
            ...
            for _ in range(30):  # Break out on fail or success. Check once per sec.
                ...
                if s.isconnected():
                    break
                if ESP32:
                    if s.status() != network.STAT_IDLE and s.status() != network.STAT_CONNECTING:  # 1000 or 1001
                        break
        ...
        if not s.isconnected():  # Timed out
            ...
            print('Wifi connected. IP:' + s.ifconfig()[0])
    ...
    async def connect(self, *, quick=False):  # Quick initial connect option for battery apps
        ...
        # loop to process message and reconnection
        asyncio.create_task(self._on_mqtt_connected())
        asyncio.create_task(self._on_message_received())
    ...
    async def subscribe_cb(self, topic, cb, qos=0):
        self._topics.append((self._topic_prefix + topic, cb))
        await self.subscribe(self._topic_prefix + topic, qos)
    ...
    async def _on_mqtt_connected(self):  # Respond to connectivity being (re)established
        while True:
            gc.collect()
            await self.up.wait()  # Wait on an Event
            self.up.clear()
            for topic, cb in self._topics:
